...
export const useWorkbenchAppsStore = defineStore('workbenchApps', () => {
  ...
  function setAppStatus(id: string, status: DevAppStatus) {
    const a = apps.value.find(x => x.id === id)
    if (a) { a.status = status; a.updatedAt = Date.now() }
  }
  function setAppsStatus(ids: string[], status: DevAppStatus) {
    const set = new Set(ids)
    const now = Date.now()
    apps.value.forEach(a => { if (set.has(a.id)) { a.status = status; a.updatedAt = now } })
  }
