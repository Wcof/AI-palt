...
## 2026-04-20 追加：ChatGPT 式设置弹窗与安全生产蓝色玻璃风

### 外壳原则
- 不再使用顶部黑色/彩色导航条。
- 一级入口全部收敛到左侧栏。
- 引擎 / Agent 切换放在主会话区左上方，不放在右上角。
- 设置类内容不做单独入口页面优先展示，点击“系统设置”后打开居中弹窗。

### 设置归类原则
- 设置弹窗内承接：品牌配置、推荐问题、Banner 管理、NL2SQL Case、专有名词、JDBC 单数据源、报告模板、内容配置。
- 左侧主菜单保留非设置能力：新会话、搜索会话、AI 技能、AI 知识库、AI 智能体、开发者工作台、近期会话。

### 视觉原则
- 交互参考 ChatGPT：左侧栏 + 主对话区 + 底部输入框 + 设置弹窗。
- 视觉参考安全生产蓝色示意图：蓝色高光、玻璃拟态、柔和大阴影、轻量数据卡片。
- 不做像素级复刻，保持极客光年安全生产平台的品牌化表达。
